...
# If using CUDA, set additional settings for performance
if device.type == "cuda":
...
class LSTMFeatureExtractor(BaseFeaturesExtractor):
    """
    Custom feature extractor that uses LSTM for sequential data processing.
    """
    def __init__(self, observation_space, features_dim=64):
        super(LSTMFeatureExtractor, self).__init__(observation_space, features_dim)
        
        n_inputs = observation_space.shape[1]  # Number of features per timestep
...
def train_model(env, total_timesteps=100000, save_path="models/rainbow_dqn"):
    """
    Train a Rainbow DQN model with LSTM feature extractor
    ...
    Returns:
        Trained model
    """
    print("Creating Rainbow DQN model with LSTM feature extractor...")
    
    # Create log directory
    os.makedirs("logs", exist_ok=True)
    
    # Create model with Rainbow DQN features
    model = DQN(
        "CnnPolicy",  # We'll override this with our custom architecture
        env,
        verbose=1,
        ...
        policy_kwargs={
            ...
            "net_arch": [64, 64],  # Additional layers after LSTM
            "normalize_images": False,
        },
        device=device  # Set the device for the model
    )
...
def evaluate_model(model, test_data_path, episodes=3):
    ...
    test_env = DummyVecEnv([lambda: DiscreteTradingEnvironment(test_df)])
    ...
    results_df.to_csv('evaluation_results_rainbow_dqn.csv', index=False)
...
if __name__ == "__main__":
    print("Starting Rainbow DQN trading agent training and evaluation...")
